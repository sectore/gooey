...
// SVG instance data (storage buffer)
// Must match GpuSvg layout exactly (80 bytes, 20 floats)
struct SvgInstance {
    // Position and size
    float pos_x;
    float pos_y;
...
layout(set = 0, binding = 0) readonly buffer SvgBuffer {
    SvgInstance svgs[];
};
...
layout(location = 2) out vec4 out_stroke_color;
layout(location = 3) out vec4 out_clip_bounds;
layout(location = 4) out vec2 out_screen_pos;
...
void main() {
    ...
    SvgInstance s = svgs[iid];
    vec2 viewport_size = vec2(viewport_width, viewport_height);

    // Calculate screen position
    vec2 pos = vec2(s.pos_x, s.pos_y) + unit * vec2(s.size_x, s.size_y);
    ...
    out_tex_coord = uv;
    out_fill_color = hsla_to_rgba(vec4(s.fill_h, s.fill_s, s.fill_l, s.fill_a));
    out_stroke_color = hsla_to_rgba(vec4(s.stroke_h, s.stroke_s, s.stroke_l, s.stroke_a));
    out_clip_bounds = vec4(s.clip_x, s.clip_y, s.clip_width, s.clip_height);
    out_screen_pos = pos;
}
