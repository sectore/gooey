...
pub fn loadFromMemory(allocator: std.mem.Allocator, data: []const u8) LoadError!DecodedImage {
    if (builtin.os.tag == .macos) {
        ...
        return loadFromMemoryLinux(allocator, data);
    } else {
...
// =============================================================================
// Linux Backend (libpng)
// =============================================================================

fn loadFromMemoryLinux(allocator: std.mem.Allocator, data: []const u8) LoadError!DecodedImage {
    // Detect format
    const format = detectFormat(data) orelse return LoadError.InvalidFormat;

    return switch (format) {
        .png => loadPngLinux(allocator, data),
...
fn loadPngLinux(allocator: std.mem.Allocator, data: []const u8) LoadError!DecodedImage {
    const png = struct {
        // libpng types
        ...
        const png_bytep = [*]u8;
        const png_bytepp = [*][*]u8;
        const png_const_bytep = [*]const u8;
        ...
        extern "c" fn png_set_read_fn(
            png_ptr: png_structp,
            io_ptr: ?*anyopaque,
            read_data_fn: *const fn (png_structp, png_bytep, usize) callconv(.c) void,
        ) void;
    ...
    const fp = c.fmemopen(@ptrCast(data.ptr), data.len, "rb") orelse {
